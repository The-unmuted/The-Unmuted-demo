...
- [ ] **P0 · Supabase Dashboard: shorten OTP expiry + verify rate limits (D-041)** — Authentication → Providers → Email → OTP Expiration: **3600 → 600** (10 min, NIST SP 800-63B). Keep OTP Length at 6. Then Authentication → Rate Limits, verify `/verify` (recommend ≤ 30/5min/IP) and `/otp` (≤ 4/hour/email). After changing the Dashboard value, update `OTP_LIFETIME_SEC` in `src/components/LoginFlow.tsx` to match so the client-side countdown stays truthful. **Client-side 5-tries-then-reset UX shipped 2026-08-12 (D-041); Dashboard change is the actual anti-brute-force fix.** Owner: Katie.
...
- [x] **D-032 UX improvements — done 2026-07-31**: titles → "TA被X了该怎么做"; 真实流程 (7 steps) + 名词解释 (4–6 glossary terms) sections added after every debrief; `SimScenario` type extended with `realFlow` + `glossary`; 72/72 tests clean.
