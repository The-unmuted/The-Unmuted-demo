...
**2026-09-03 final result-card state:** All three simulations share one fixed 1080×1020 Canvas result-card renderer. The compact card keeps the brand header and top-right beta QR, score/100 (with `/100` omitted at exactly 100), status pill, centered logo ring, four-band legend, and one “场景科普小 Tip”. Domestic-violence and sexual-harassment tips live in `src/data/simulationTips.json`, transcribed from the supplied reviewed PDFs and selected deterministically from scenario id + sorted result flags; sexual-assault falls back to the first visited legal coach hint until its tip source file is supplied. Chinese wrapping removes hard line breaks, keeps punctuation off line starts, and protects common short words and number-unit combinations. The earlier 3+3/P2 layouts below are historical and superseded by this entry.

**2026-09-03 (result-card balance follow-up):** The Canvas is now 1080×1605. Both 3-item columns render a purpose-written, approximately 20-character supporting line under every title; personalized feedback no longer relies on ellipsis truncation. The header's Chinese and English brand lines have more separation. The footer follows the approved P2 composition: “你不是一个人” plus the retained brand promise on the left, two-line beta label plus 123px QR on the right, and a centered bilingual signature. This supersedes the earlier dimensions in the entry below.

**2026-09-03 (unified simulation result card):** All three simulations now auto-render the same 1080×1485 score/report image directly below the ending summary; the separate “save shareable card” disclosure was removed, and the image opts back into iOS long-press saving. The reference visual was rebuilt as a layered system rather than a flat Canvas: deep-purple mesh glows, deterministic fine grain, gradient glass panels, top-edge highlights, deeper shadows, dotted ring guide, layered ring glow, illuminated endpoint, and more deliberate spacing. Each scenario has a specific “安全应对 · 知识储备得分” title and the in-card line “长按保存图片 · 希望这些知识永远不必用上”. Score colors are now brighter but controlled: 0–39 coral red, 40–59 luminous yellow, 60–79 soft purple, 80–100 fresh green. The score, status pill, progress ring, and active legend band share the band's color. Both header and ring use the real `/the-unmuted-mark.png`; its visible alpha bounds are measured before drawing so the mark is optically centered rather than centered by transparent PNG padding. The lower card remains a two-column 3+3 layout. Education now prioritizes good steps missed on this run and converts them into neutral knowledge labels (never “你……” phrasing), with concise, purpose-written explanations and three-line room; triggered mistakes/glossary/flow remain fallbacks. The small beta QR code sits in the saved image's bottom-right corner. Existing detailed analysis remains below “查看具体分析”. 82/82 Vitest tests, scoped ESLint, and Vite production build pass; local-browser low-score card verified with no console errors. Build still reports the pre-existing duplicate `no-record` score-key warning.
